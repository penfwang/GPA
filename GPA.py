...
import resources_mine,mutation_crossover
...
def main(seed,dataset_name):
 ...
 ##################################using all features
    ...
    toolbox.register("mate", mutation_crossover.cxOnePoint)
    toolbox.register("expr_mut", gp.genFull, min_=0, max_=3)
    toolbox.register("mutate", mutation_crossover.mutUniform, expr=toolbox.expr_mut, pset=pset)
 ...
 # ngen: The number of generation.
    archive, pop, log = resources_mine.eaSimple(ee,pset, pop, toolbox, cxpb=0.8, mutpb=0.2, elitismNum=1, ngen=100, stats=mstats, halloffame=hof, verbose=True)
    function = toolbox.compile(expr=hof[0])
    # print(hof[0])
    return archive,pop,hof[0]
...
if __name__ == "__main__":
    ...
    start = time.time()
    archive,pop,p_one= main(seed, dataset_name)
    end = time.time()
    running_time = end - start
    #saveFile.save_individual(seed, dataset_name, p_one)
    saveFile.save_archive(seed, dataset_name, archive)
    saveFile.save_pop(seed, dataset_name, pop)
